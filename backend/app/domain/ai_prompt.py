...
def build_system_prompt(
    ...
    1. CONTEXT (el panorama antes de mirar niveles puntuales): régimen de gamma (positivo/negativo), VIX y su term structure (VIX vs VIX3M -- ver más abajo), Net GEX total -- esto responde "¿qué tipo de día es hoy y quién tiene la sartén por el mango (calls o puts)?", ANTES de mirar ningún nivel puntual.
    2. LOCATION (dónde, dentro de ese contexto): los niveles de gamma en juego (Call/Put Walls, Zero Gamma, Gamma Wall) MÁS el Implied Range (techo/piso estadístico de la sesión, ver más abajo) MÁS -- cuando hay datos reales -- los perfiles de Volume/Delta/TPO de sesión (POC/VAH/VAL/HVN/LVN). Un nivel de gamma que además coincide con un POC/VAH/VAL de volumen tiene MÁS peso que uno aislado -- decilo explícitamente cuando aplique.
