...
from app.domain.gex_math import compute_call_put_walls
...
def build_live_levels_payload(df: pd.DataFrame, spot_price: float, conversion_ratio: float) -> dict | None:
    """Arma el payload exacto que espera GexProfileCloud.cs (gamma_schwab.txt)
    en el nodo /live_levels de Firebase: {qqq_spot, conversion_ratio,
    cw1..cw3, pw1..pw3, levels: [{strike, net_gex}]}. Port de la sección
    final de export_live_levels_to_quantower en app.py (~línea 1685):
