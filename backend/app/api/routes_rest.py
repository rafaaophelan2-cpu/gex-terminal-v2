...
async def get_drift(
    ...
    'mode': "gex" (default, compute_drift_series -- reconstruido de Open
    Interest) o "volume_premium" (compute_volume_premium_drift_series --
    PROTOTIPO/EXPERIMENTAL, reconstruido del premium real operado vía
    delta de volumen entre snapshots -- agregado 14-sep-2026 para
    comparar en vivo contra un Net Drift real antes de decidir si
    reemplaza al de arriba). Un 'mode' desconocido cae al comportamiento
    de "gex" -- nunca deja al frontend sin datos por un typo en el query
    param."""
    symbol = _normalize_symbol(symbol)
    snapshots = await _fetch_day_snapshots(symbol, date)
    if mode == "volume_premium":
        return compute_volume_premium_drift_series(snapshots, otm_only=otm_only)
